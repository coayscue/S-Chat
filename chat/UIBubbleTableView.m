...
//    }
    ...
    //returns size of bubble + 3 for a small gap bellow each bubble
    return MAX(data.insets.top + data.view.frame.size.height + data.insets.bottom + 3 , self.showAvatars ? 52 : 0);
}
...
//    }
    ...
    //check for what the next bubble's type is: if its the same type, make this data's type the 2 version; if its of a different type, or if it is the last row in the section, make the type the regular
    
    NSArray *section = [self.bubbleSection objectAtIndex:indexPath.section];
    
    if (section.count != indexPath.row) {
        NSBubbleData *nextData = [section objectAtIndex:indexPath.row - 1];
        switch (data.type) {
            case BubbleTypeMine:
                if ((nextData.type == BubbleTypeMine || nextData.type == BubbleType2Mine)) {
                    data = [NSBubbleData dataWithView:data.view date:data.date type:BubbleType2Mine insets:data.insets];
                }else{
                    data = [NSBubbleData dataWithView:data.view date:data.date type:BubbleTypeMine insets:data.insets];
                }
                break;
            ...
            case BubbleTypeSomeoneElse:
                if ((nextData.type == BubbleTypeSomeoneElse || nextData.type == BubbleTypeSomeoneElse)) {
                    data = [NSBubbleData dataWithView:data.view date:data.date type:BubbleType2SomeoneElse insets:data.insets];
                }else{
                    data = [NSBubbleData dataWithView:data.view date:data.date type:BubbleTypeSomeoneElse insets:data.insets];
                }
                break;
            case BubbleType2SomeoneElse:
                if ((nextData.type == BubbleTypeSomeoneElse || nextData.type == BubbleTypeSomeoneElse)) {
                }else{
                    data.type = BubbleTypeMine;
                }
            default:
                break;
        }
